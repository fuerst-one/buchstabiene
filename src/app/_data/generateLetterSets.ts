...
import { readWordFile, writeWordFile } from "./files";
...
export const generateLetterSets = (allWords: string[]) => {
...
const main = async () => {
  ...
  const letterSets = generateLetterSets(words);
  await writeWordFile("letterSets.txt", letterSets);
};
