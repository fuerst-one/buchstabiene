...
export const Downvotes = ({ downvotes }: { downvotes: WordVote[] }) => {
  ...
  const downvotesByWord = downvotes
    .reduce(
      (acc, vote) => {
        if (vote.vote >= 0) {
          return acc;
  ...
  const deleteSelectedWords = async () => {
    const deleted = await adminDeleteWordFromGames(selectedWords);
    console.log(deleted);
    // setSelectedWords([]);
  };
  ...
  return (
    <div className="my-1 overflow-y-auto rounded-sm bg-white/10 px-2 py-1">
      {downvotesByWord.length ? (
        <>
          {downvotesByWord.map(({ word, count, users }) => (
            <div
              key={word}
            ...
            >
              <span>
                <Trash
                  className={cn(
                    "size-3",
                    selectedWords.includes(word) && "text-red-500",
                  )}
                />
              </span>
          ...
          <div className="mt-2 flex gap-2">
            <Button
              onClick={() =>
                setSelectedWords(downvotesByWord.map(({ word }) => word))
              }
            >
              Select All
            </Button>
            <Button onClick={deleteSelectedWords}>Delete selected</Button>
          </div>
        </>
      ) : (
        <p>No downvotes found</p>
      )}
    </div>
  );
};
